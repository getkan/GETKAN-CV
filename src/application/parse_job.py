...
from src.application.skill_rules import SKILL_BLOCKLIST_PATTERNS, SKILL_CANONICAL_MAP, _SKILL_PREFIXES
...
def fetch_or_load_listing(state: JobParserState) -> JobParserState:
    ...
    try:
        ...
        structured_text = _structured_listing_text(payload)
        cleaned = re.sub(r"<script.*?</script>", " ", payload, flags=re.S | re.I)
        cleaned = re.sub(r"<style.*?</style>", " ", cleaned, flags=re.S | re.I)
        cleaned = re.sub(r"<[^>]+>", "\n", cleaned)
        cleaned = unescape(cleaned)
        cleaned = re.sub(r"\s+", "\n", cleaned).strip()
        state["raw_listing_text"] = structured_text or cleaned
...
def extract_facts(state: JobParserState) -> JobParserState:
    ...
    source_text = "\n".join(part for part in (text, html_text) if part)
    fallback_must_have = _source_skill_mentions(source_text)
    model_must_have = _source_supported_skills(payload.get("must_have") or [], source_text)
    model_nice_to_have = _source_supported_skills(payload.get("nice_to_have") or [], source_text)
    fallback_nice_to_have = _source_supported_skills(fallback_payload.get("nice_to_have") or [], source_text)
    ...
    merged_payload = {
        ...
        "must_have": _merge_skill_items(model_must_have, fallback_must_have, fallback_payload.get("must_have") or []),
        "nice_to_have": _merge_skill_items(model_nice_to_have, fallback_nice_to_have),
...
def _contains_keyword(text: str, keyword: str) -> bool:
    ...
    return re.search(rf"(?<![A-Za-z0-9]){escaped}(?![A-Za-z0-9])", text, flags=re.I) is not None
...
def _structured_listing_text(html: str) -> str:
    descriptions: list[str] = []
    for posting in _extract_jsonld_job_postings(html):
        description = posting.get("description")
        if isinstance(description, str) and description.strip():
            descriptions.append(_strip_html(description))
    return "\n\n".join(descriptions).strip()


def _source_supported_skills(items: list[str] | None, source_text: str) -> list[str]:
    supported: list[str] = []
    for item in normalize_skill_items(items):
        if item.lower() == "solid":
            continue
        aliases = [item]
        aliases.extend(alias for alias, canonical in SKILL_CANONICAL_MAP.items() if canonical.lower() == item.lower())
        if any(_contains_keyword(source_text, alias) for alias in aliases):
            supported.append(item)
    return normalize_skill_items(supported)


def _source_skill_mentions(source_text: str) -> list[str]:
    mentions: list[str] = []
    ignored = {"c", "ci", "cd", "net"}
    for alias, canonical in sorted(SKILL_CANONICAL_MAP.items(), key=lambda entry: len(entry[0]), reverse=True):
        if alias in ignored or not _contains_keyword(source_text, alias):
            continue
...
def _merge_skill_items(*groups: list[str]) -> list[str]:
    merged: list[str] = []
    for group in groups:
        merged.extend(group)
    return normalize_skill_items(merged)
    ...
    if job_postings:
        ...
        if not must_have:
            ...
            for keyword in [
                ...
                if _contains_keyword(qualifications_lower, keyword):
